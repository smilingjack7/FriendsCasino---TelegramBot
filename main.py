...
def get_hand_value(h: list) -> int:
    ...
    return v
def format_hand(h:list, hide_one:bool=False)->str: # Use hide_one parameter name
    if not h: return "Пусто"
    if hide_one and len(h)>1: return f"[{h[0][0]}{h[0][1]}, ??]" if h[0] else "[??, ??]" # Use hide_one variable
...
async def blackjack_start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_game and user_game.get('message_id') and delete_prev_msg_id != user_game.get('message_id'):
        try: await context.bot.delete_message(chat.id, user_game.get('message_id'))
        except: pass
    balance=get_balance(user.id)
    ...
    try:
        if delete_prev_msg_id: await context.bot.delete_message(chat.id, delete_prev_msg_id); sent=await context.bot.send_message(chat.id, text, reply_markup=markup, parse_mode=ParseMode.HTML)
        else: sent=await reply_func(text, reply_markup=markup, parse_mode=ParseMode.HTML)
...
async def blackjack_handle_bet(update: Update, context: ContextTypes.DEFAULT_TYPE, bet: int):
    q=update.callback_query; u=q.from_user; uid=u.id; game=context.user_data.get(BJ_GAME_KEY,{})
    if not game or game.get('state') != 'waiting_bet': await q.answer("Не время.", show_alert=False); return
    bal=get_balance(uid)
    ...
    await blackjack_show_state(context, uid, q.message.message_id); await q.answer(f"Ставка {bet} F!")

async def blackjack_show_state(context: ContextTypes.DEFAULT_TYPE, user_id: int, msg_id: int):
    ...
    try: await context.bot.edit_message_text(user_id, msg_id, txt, reply_markup=mrk, parse_mode=ParseMode.HTML)
    except BadRequest as e:
        if"message is not modified" not in str(e).lower(): logger.warning(f"Edit BJ fail {msg_id} u {user_id}: {e}")
    except Exception as e: logger.error(f"Show BJ error u {user_id}: {e}")
...
async def blackjack_handle_action(update: Update, context: ContextTypes.DEFAULT_TYPE, parts: list):
    ...
    try: # Main action block
        if act=='hit':
            ...
            if sc==0 and c:
                ...
                if hv>21: hd['status']='bust'; await blackjack_next_action(context, uid)
                elif hv==21: hd['status']='stand'; await blackjack_next_action(context, uid)
                else: upd=True
            else: raise IndexError("Draw fail") # Trigger except block if draw fails
        elif act=='stand':
            hd['status']='stand'; await q.answer("Стоп."); await blackjack_next_action(context, uid)
        elif act=='double':
            ...
            if can and update_balance(uid, -b) is not None:
                ...
                await blackjack_next_action(context, uid)
             ...
             if can and bal is not None and bal>=b and update_balance(uid, -b) is not None:
                 ...
                 if is_a:
                     hd['status']=nh['status']='stand'; hd['can_double']=nh['can_double']=False; await q.answer("Тузы разд."); await blackjack_next_action(context, uid)
                 else:
                     # Update flags for both hands after split
    ...
    # --- Exception Handling for Actions ---
         ...
         hd['status']='stand'; await q.answer("Не взять карту!",show_alert=True); await blackjack_next_action(context, uid)
    except Exception as e: logger.error(f"BJ act '{act}' u {uid}: {e}", exc_info=True); await q.answer("Ошибка.",show_alert=True)
    # Update game state message if needed (e.g., after hit/split that didn't end the turn)
    if upd: await blackjack_show_state(context, uid, q.message.message_id)

async def blackjack_next_action(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    game=context.application.user_data.get(user_id, {}).get(BJ_GAME_KEY)
    if not game or game.get('state') != 'player_turn': return
    ...
    if ni!=-1: game['current_hand_index']=ni; await blackjack_show_state(context, user_id, game['message_id'])
    else: game['state']='dealer_turn'; await blackjack_show_state(context, user_id, game['message_id']); context.job_queue.run_once(blackjack_dealer_turn_job, DEALER_TURN_DELAY, data=user_id, name=f"dealer_{user_id}")

async def blackjack_dealer_turn_job(context: ContextTypes.DEFAULT_TYPE):
    uid=context.job.data; game=context.application.user_data.get(uid, {}).get(BJ_GAME_KEY)
    ...
    if not can_w and not d_bj: await blackjack_determine_outcome(context, uid, d_bj); return
    while True:
        ...
        if dv>17 or (dv==17 and not (soft and DEALER_HITS_SOFT_17)):
            logger.info(f"Дилер стоп на {dv} ({format_hand(dh)}) у {uid}.") # Log dealer stand
            break
        # --- Исправленный блок try-except ---
        try:
            c, sc = _get_next_item(dk, dlt, NUM_DECKS)
            if sc == 0 and c: # Check status and card
                dh.append(c); game['cards_dealt'] += 1; dlt = game['cards_dealt']
            else:
                raise IndexError("Dealer draw fail") # Card is None or status is error
        except IndexError:
            logger.warning(f"BJ Dealer {uid} остановился - колода пуста?")
            break # Exit while loop if draw fails
        # --- Конец исправления ---
    await blackjack_determine_outcome(context, uid, d_bj)

async def blackjack_determine_outcome(context: ContextTypes.DEFAULT_TYPE, user_id: int, d_had_bj: bool):
    game=context.application.user_data.get(user_id, {}).get(BJ_GAME_KEY)
    if not game: return
    if game.get('state')=='game_over' and 'outcome_determined' in game: await blackjack_show_state(context, user_id, game['message_id']); return
    ...
    await blackjack_show_state(context, user_id, game.get('message_id'))
